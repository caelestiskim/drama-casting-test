  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-5xl px-6 py-8 sm:px-10">
      <header className="flex items-center justify-between py-4">
        <div>
          <p className="text-sm font-semibold tracking-[0.2em] text-cyan-700">{copy.nav.privacy}</p>
          <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">
            {copy.privacyPage.title}
          </h1>
        </div>
        <div className="flex items-center gap-3">
          <LocaleSwitcher locale={locale} path="/privacy" />
          <Link
            href={`/${locale}`}
      ...
      <section className="mt-8 rounded-[2rem] border border-slate-200 bg-white/92 p-8 shadow-[0_20px_70px_rgba(15,23,42,0.06)]">
        <p className="text-sm leading-7 text-slate-600">{copy.privacyPage.intro}</p>
      </section>

      <section className="mt-8 grid gap-5">
        {copy.privacyPage.sections.map((section) => (
          <article
            key={section.title}
